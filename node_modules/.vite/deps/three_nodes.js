...
var TextureNode = class extends UniformNode_default {
  ...
  getDefaultUV() {
    ...
    return uniform(texture2.matrix).mul(vec3(uv(texture2.channel), 1));
  }
  construct(builder) {
...
var MaterialNode = class _MaterialNode extends Node_default {
  ...
  getNodeType(builder) {
    ...
    if (scope === _MaterialNode.COLOR) {
      ...
      return "float";
    } else if (scope === _MaterialNode.EMISSIVE || scope === _MaterialNode.SHEEN) {
      return "vec3";
    } else if (scope === _MaterialNode.ROUGHNESS || scope === _MaterialNode.METALNESS || scope === _MaterialNode.SPECULAR || scope === _MaterialNode.SHININESS || scope === _MaterialNode.CLEARCOAT_ROUGHNESS || scope === _MaterialNode.SHEEN_ROUGHNESS) {
      return "float";
    }
  }
  getFloat(property2) {
...
var perturbNormal2ArbNode = tslFn((inputs) => {
...
var AnalyticLightNode = class extends LightingNode_default {
  ...
  constructShadow(builder) {
    ...
    if (shadowNode === null) {
      ...
      let depth2 = texture(depthTexture2, shadowCoord.xy);
      depth2 = depth2.mul(0.5).add(0.5).add(bias);
      shadowNode = cond(shadowCoord.z.lessThan(depth2).or(
...
var NodeMaterial = class _NodeMaterial extends ShaderMaterial {
  ...
  get isUnlit() {
    return this.constructor === _NodeMaterial.prototype.constructor;
  }
  copy(source) {
...
var LightingContextNode = class extends ContextNode_default {
  ...
  construct(builder) {
    ...
    if (lightingModelNode && lightingModelNode.init)
      lightingModelNode.init(context2, builder.stack, builder);
    if (lightingModelNode && lightingModelNode.indirectDiffuse)
      lightingModelNode.indirectDiffuse(context2, builder.stack, builder);
    if (lightingModelNode && lightingModelNode.indirectSpecular)
      lightingModelNode.indirectSpecular(context2, builder.stack, builder);
    if (lightingModelNode && lightingModelNode.ambientOcclusion)
      lightingModelNode.ambientOcclusion(context2, builder.stack, builder);
    return super.construct(builder);
  }
  generate(builder) {
...
var RE_Direct_BlinnPhong = tslFn(({ lightDirection, lightColor, reflectedLight }) => {
  ...
  reflectedLight.directSpecular.addAssign(irradiance.mul(BRDF_BlinnPhong_default({ lightDirection })).mul(materialReflectivity));
});
var RE_IndirectDiffuse_BlinnPhong = tslFn(({ irradiance, reflectedLight }) => {
  reflectedLight.indirectDiffuse.addAssign(irradiance.mul(BRDF_Lambert_default({ diffuseColor })));
});
var phongLightingModel = lightingModel(null, RE_Direct_BlinnPhong, RE_IndirectDiffuse_BlinnPhong);
...
var BRDF_GGX = tslFn((inputs) => {
  ...
  const dotVH = positionViewDirection.dot(halfDir).clamp();
  const F = F_Schlick_default({ f0, f90, dotVH });
...
var LM_Init = tslFn((context2, stack2, builder) => {
  if (builder.includes(clearcoat)) {
    context2.clearcoatRadiance = vec3().temp();
    context2.reflectedLight.clearcoatSpecular = vec3().temp();
    ...
    const outgoingLight = context2.reflectedLight.total;
    const clearcoatLight = outgoingLight.mul(clearcoat.mul(Fcc).oneMinus()).add(context2.reflectedLight.clearcoatSpecular.mul(clearcoat));
    outgoingLight.assign(clearcoatLight);
  }
  if (builder.includes(sheen)) {
    context2.reflectedLight.sheenSpecular = vec3().temp();
    const outgoingLight = context2.reflectedLight.total;
    const sheenEnergyComp = sheen.r.max(sheen.g).max(sheen.b).mul(0.157).oneMinus();
    const sheenLight = outgoingLight.mul(sheenEnergyComp).add(context2.reflectedLight.sheenSpecular);
    outgoingLight.assign(sheenLight);
  }
});
var RE_IndirectSpecular_Physical = tslFn((context2) => {
  const { radiance, iblIrradiance, reflectedLight } = context2;
  ...
  if (reflectedLight.clearcoatSpecular) {
    ...
    reflectedLight.clearcoatSpecular.addAssign(context2.clearcoatRadiance.mul(clearcoatEnv));
  }
...
var RE_IndirectDiffuse_Physical = tslFn((context2) => {
  const { irradiance, reflectedLight } = context2;
  reflectedLight.indirectDiffuse.addAssign(irradiance.mul(BRDF_Lambert_default({ diffuseColor })));
});
var RE_Direct_Physical = tslFn((inputs) => {
  const { lightDirection, lightColor, reflectedLight } = inputs;
  const dotNL = transformedNormalView.dot(lightDirection).clamp();
  const irradiance = dotNL.mul(lightColor);
  if (reflectedLight.sheenSpecular) {
    reflectedLight.sheenSpecular.addAssign(irradiance.mul(BRDF_Sheen_default({ lightDirection })));
  }
  if (reflectedLight.clearcoatSpecular) {
    const dotNLcc = transformedClearcoatNormalView.dot(lightDirection).clamp();
    const ccIrradiance = dotNLcc.mul(lightColor);
    reflectedLight.clearcoatSpecular.addAssign(ccIrradiance.mul(BRDF_GGX_default({ lightDirection, f0: clearcoatF0, f90: clearcoatF90, roughness: clearcoatRoughness, normalView: transformedClearcoatNormalView })));
  }
  reflectedLight.directDiffuse.addAssign(irradiance.mul(BRDF_Lambert_default({ diffuseColor: diffuseColor.rgb })));
  reflectedLight.directSpecular.addAssign(irradiance.mul(BRDF_GGX_default({ lightDirection, f0: specularColor, f90: 1, roughness })));
});
var RE_AmbientOcclusion_Physical = tslFn((context2) => {
  const { ambientOcclusion, reflectedLight } = context2;
  const dotNV = transformedNormalView.dot(positionViewDirection).clamp();
...
var physicalLightingModel = lightingModel(LM_Init, RE_Direct_Physical, RE_IndirectDiffuse_Physical, RE_IndirectSpecular_Physical, RE_AmbientOcclusion_Physical);
